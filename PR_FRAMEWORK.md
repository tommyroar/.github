...
**Generation happens once, by the agent** that opens or updates the PR — there is no
per-push `claude -p` regeneration anymore (the old global pre-push hook was retired
2026-06-04). When an agent pushes follow-up commits to an open PR, it rebuilds the body
then. For a **human** push with no agent in the loop, run `refresh_pr.sh <n>` (next to
this file) on demand: it regenerates the body from the full diff, validates it, and edits
the PR.

Enforcement is GitHub-native and model-free: repos carry a `.github/pull_request_template.md`
(the skeleton) and a `pr-newspaper` workflow that runs `validate_pr.py` on the PR body
(readability + page budget) — so a malformed body fails CI without re-running Claude.
